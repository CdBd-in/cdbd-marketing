...
| 4 | 데이터 | **Body-Half** (이미지↑/텍스트↓) | 소제목: 한 페이지에 다 담았다 · 본문: 초대장 + 룩북 + RSVP 응답 카드까지, 하나의 모바일 페이지로. | `allincomplete-lookbook1.jpg` (라이트블루 볼캡 룩북) |
...
3. **4번 데이터 페이지** — 본래 "CdBd로 만든 모바일 한 페이지"를 보여주는 게 가장 강함. 지금은 룩북 화보로 대체 → **CdBd 실제 제작 페이지 스크린샷(보유 자산)** 이 있으면 교체 권장.
