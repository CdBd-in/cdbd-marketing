...
updated: 2026-05-26
related:
  - "[[블로그/1. 썸네일 디자인 시스템/썸네일 디자인 시스템]]"
---

# CdBd 소재집

> **목적**: 블로그 썸네일 자동 생성 시 Claude가 타이틀에 맞는 적절한 시각 자산을 *자동 선택*할 수 있도록 한 카탈로그.
> **사용처**: [[블로그/1. 썸네일 디자인 시스템/썸네일 디자인 시스템]] 섹션 8 프롬프트 템플릿과 함께 컨텍스트로 전달.
...
#### 1c. 콘텐츠 형식 추출 → 비주얼 조합 결정
타이틀의 콘텐츠 유형이 무엇인지.

| 패턴 | 분류 | 주 비주얼 |
|------|------|----------|
| "X 만들고", "X로", "X 활용법", "X 사용 사례", "X 시리즈" | **템플릿 데모** | 목업 1~3개 (전체 viewer) |
| "X 기능", "X 업데이트", "새로운 X", "X 추가" | **기능 강조** | 목업 1개 (기능 영역 zoom) |
| "X란?", "왜 X?", "X 대신", "X vs Y" | **컨셉 비교** | 3D 오브젝트 + 텍스트 강조 |
| "X 시리즈 #N", "(카테고리명) 시리즈" | **시리즈** | 목업 1개 + 시리즈 라벨 (서브타이틀) |
...
1. **1a (기능명) 매칭 있으면** → § 3에서 직접 캡처 소스 결정 (가장 명시적)
2. **1b (산업) 매칭 있으면** → § 2에서 산업 일치 슬러그 1개 + 1d 데코 조합
3. **1a·1b 모두 없으면** → 1c 형식만으로 결정 (목업 일반 / 3D 오브젝트 등)
4. **1d (톤)은 항상 추출** → 데코·이모지 선택에 사용
...
- 1a: "프로필 링크" → § 3 프로필 링크 기능 (캡처 소스: `profilelink/promotion` 상단)
- 1b: 매칭 없음 (마케터는 일반 직군)
- 1c: "만들어봐" → 템플릿 데모
- 1d: 매칭 없음 → 기본 sparkle
- **결정**: `profilelink/promotion` viewer 전체 목업 + sparkle

**예시 2**: "**VIP 1:1 예약 관리**, 전화 핑퐁 모바일 초대장으로 끝내기"
- 1a: "예약 관리" → § 3 예약 캘린더 (`invitation/reservation` 중단)
- 1b: "VIP" → § 2 `invitation/seminar` 가능성 (VIP)
- 1c: "끝내기" → 컨셉 비교 (전화 vs 모바일)
- 1d: "VIP" → crown
- **결정**: `invitation/reservation` 예약 캘린더 zoom + crown 데코

**예시 3**: "**레이어 추가하기** 기능 업데이트"
- 1a: "레이어" → § 3.2 에디터 편집 화면
- 1b: 매칭 없음
- 1c: "기능 업데이트" → 기능 강조
- 1d: 매칭 없음 → sparkle
- **결정**: § 3.2 레이어 추가 캡처 + sparkle (✦ "New" 톤)

**예시 4**: "모바일 명함과 브로셔 만들고 팀 전체에 공유하는 방법" (오늘 작업)
- 1a: 매칭 없음 (명함·브로셔는 카테고리명)
- 1b: 매칭 없음
- 1c: "만들고...방법" → 템플릿 데모 (다중 - 명함+브로셔)
- 1d: "팀 전체" → Memoji 그룹 (실제로는 sparkle 사용했음 — 다음엔 Memoji 추천)
- **결정**: `profilelink` 1개 + `catalog` 1개 목업 2개 한 덩어리 + Memoji ✅
