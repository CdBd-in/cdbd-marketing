...
> - **2·3·4번 (`lookbook`·`newarrival`·`online_lookbook`)** → **멀티페이지 목업** 사용 — 캡처는 중앙 #FFFFFF 영역에 fill
> ⚠️ 새 파일(`3LPfZ6a6vlJrZ4TXorVWJL`)에 멀티페이지 목업 공식 컴포넌트 없음 → 슬롯 페이지의 멀티 슬롯(`VISUAL_SLOT_1/2/3`) 안에 직접 캡처 fill 권장 — [[1-3-1. 이미지 규칙]] §1.1.a 참고
...
| 2026-05-31 | Figma 파일 마이그레이션 — 옛 파일(`1gduKu9Bg319AMqgFrgjTw`) → 새 파일(`3LPfZ6a6vlJrZ4TXorVWJL`, "CdBd 블로그 썸네일"). 슬롯 페이지(`1:1245`) + 60개 완성 썸네일(`0:1`) 구조. 별도 공식 목업 컴포넌트 페이지 없음. |
