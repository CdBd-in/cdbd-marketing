...
> **역할**: [[1-3-1. 이미지 규칙]]이 정의한 이미지 슬롯(주/보조)에 **실제로 넣을 자산**을 결정. (규칙=어떻게 / 이미지 아카이브=무엇을)
...
| 일자 | 변경 내용 |
|------|-----------|
| 2026-05-26 | 이미지 아카이브 초기 생성 — 템플릿 3개(promotion, oak_table, newarrival) + 데코 1개(sparkle) + 매핑 사전 + Figma fills 적용 주의사항 |
...
- [ ] 이미지 아카이브 → JSON 변환 스크립트 (프로그래매틱 활용)
