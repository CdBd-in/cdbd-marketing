...
> ℹ️ 원본 스크랩 이미지(`_downloads/`, 286MB)는 분석 완료 후 **삭제**됨 — 결론은 이 문서와 [[_분석원본_게시물별 이미지 분류.md]]에 distilled. 필요 시 블로그에서 재수집 가능.
...
> 분석에 사용한 스크랩 원본 390장(`_downloads/`, 286MB)은 **분석 완료 후 삭제**됨 (자동화 파이프라인은 Figma 슬롯+viewer 캡처를 쓰므로 불필요).
> 게시물별 이미지 분류 결과는 [[_분석원본_게시물별 이미지 분류.md]]에 전부 보존돼 있고, 필요 시 블로그(home.cdbd.in)에서 동일 방식으로 재수집 가능.
