...
> - **에디터·관리자 캡처** (로그인 필요): 썸네일 [[1-1-3. 이미지 규칙]] §3.2
> - **블로그 본문 이미지 viewer 섹션 캡처** (로그인 불필요): [[1-2-2-2. 이미지 제작 가이드]] §4.1
...
### ⚠️ 핵심 규칙 (가이드 [[1-1-3. 이미지 규칙|1-3. 이미지 규칙]] §3.3.1)
> ⚠️ **2026-06-25 멀티페이지 목업(`338:3238`) 재설계** — fill 대상이 구 `Rectangle 3424`에서 내부 `Change-This` 노드(`338:3245`·`338:3246`)로 변경. 아래 비율·노드명은 구 컴포넌트 기준 → **스크립트 실행 전 새 컴포넌트로 재확인 필요.**
- **fill 대상 = 멀티페이지 목업의 #ffffff screen 영역**(`Change-This`) ⭐
- **폰 본체 기본 이미지 → 절대 건드리지 않음** (마스터 기본 유지)
- 캡처 비율은 screen 영역 비율에 맞춤 → FILL 시 잘림·왜곡 0
...
2. C 슬롯(`338:3150`) clone → VISUAL_SLOT_1/2/3에 멀티페이지 목업(`338:3238`) 인스턴스
...
## 🌐 영문(다국어) 버전 캡처 — DOM 텍스트 주입 (NEW 2026-09-07)

> 뷰어 템플릿은 **한국어 전용**이다(`locale: en-US` 로 띄워도 한글). 영문 버전이 필요하면
> **DOM 텍스트 노드를 EN으로 치환한 뒤 캡처**한다. → `capture-seminar-venue-EN.mjs`

### 국문 캡처와 "같은 프레이밍"으로 맞추는 법

1. Figma의 국문 이미지를 `get_screenshot` 으로 원본 해상도(422×911) 받아서 **랜드마크 y좌표 실측**
   (파란 섹션 타이틀·지도 핀·주소·버튼 밴드)
2. 배율 = `Figma 폭 / 캡처 viewport 폭` (이 건 `422/378 = 1.1164`)
3. `ANCHOR` = 클립 상단에서 기준 요소(rect top)까지의 거리 → `clipTop = 기준요소Y - ANCHOR`
   실측·재캡처를 1~2회 돌려 **랜드마크 오차 1px 이내**로 수렴시킨다
4. Figma rect는 **캡처와 정확히 같은 종횡비로 resize** 후 `FILL`
   (`h = w × 캡처H / 캡처W`) — 안 맞추면 FILL이 crop 하면서 정렬이 어긋난다

### 🔴 함정 2가지 (실제로 걸림)

| 함정 | 내용 | 해결 |
|---|---|---|
| **지도 인스턴스가 2개** | 이 페이지는 `naver.maps.Map` 을 **2번** 생성한다 — 보이는 것(266×177)과 숨은 것(380×253). 후킹해서 아무거나 `setZoom` 하면 **숨은 쪽만 바뀌고 화면은 그대로** | `getElement()` 의 `getBoundingClientRect().width > 0` 인 것만 골라 `setZoom` |
| **스크롤하면 지도가 리셋** | 최종 위치로 스크롤한 뒤 zoom 을 걸어야 한다. zoom 먼저 → 스크롤 하면 zoom 15로 되돌아감 | **스크롤 → setZoom → 타일 대기(8s) → 캡처** 순서 고정 |

- 지도 확대 배율: **zoom 17 = 스케일바 50m** (국문 원본과 동일). 기본값은 zoom 15 = 300m.
- 지도 조작(더블클릭·휠)은 **비활성화**되어 있어 제스처로는 못 바꾼다 → 인스턴스 후킹이 유일한 방법.

### 🔴 영문 카피 — 본문 컬럼 폭 **266px** 를 넘기면 줄이 늘어난다

줄이 늘어나면 아래 요소가 전부 밀려 **국문과 프레이밍이 어긋난다.** 그래서:

- 스크립트가 치환 후 `Range.getClientRects().length > 1` 로 **wrap 을 자동 검증**한다 (`wrapped: []` 여야 정상)
- 실측 상한 — 본문 14px: 약 34자 / 주소·장소 16px: 약 30자
- 카피는 [[CLAUDE.md]] §4 원칙대로 **골격 유지·명사만 교체**
  예) `강남구 테헤란로5길 7 KG Tower`(213px) → `KG Tower, 7 Teheran-ro 5-gil`(209px) — 폭까지 맞고 고유명사 보존
  (`7 Teheran-ro 5-gil, Gangnam-gu` 236px 도 1줄이지만 건물명이 빠진다)
- 톤은 기존 `screenshots/usecase06/en/seminar-cover-EN.png` 과 통일 (British 철자 `honour`, VVIP 의전체)
