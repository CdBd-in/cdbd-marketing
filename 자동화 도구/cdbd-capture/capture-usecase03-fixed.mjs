...
const OUT=resolve('./screenshots/usecase03'); if(!existsSync(OUT)) mkdirSync(OUT,{recursive:true});
const SHOTS=[
  {name:'p-program', y:2016},   // 주요 프로그램 헤딩(2076) - 60
  {name:'p-rsvp',    y:2959},   // 참석 및 사전 정보 등록(3019) - 60
  {name:'p-venue',   y:1022},   // 일시(1082) - 60
];
...
for(const s of SHOTS){
  ...
  await p.screenshot({path:join(OUT,`seminar430-${s.name}.png`),clip:{x:0,y:0,width:W,height:H}});
