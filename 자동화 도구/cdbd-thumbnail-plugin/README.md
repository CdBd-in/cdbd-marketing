...
[[0. 목표와 비전]] §⚙️ 파이프라인의 **5단계 (N안 자동 제작)** 를 담당하는 Figma 데스크탑 플러그인. `use_figma` (Figma MCP)가 변경을 commit하지 못하는 한계를 우회한다.
...
3. UI에 다음 입력:
   ...
   - **강조어** — 타이틀 안의 강조 문구 ([[1-1. 스타일]] 강조 키워드 선택 원칙 참고)
   - **변형 매트릭스 JSON** — Claude가 분석으로 제안한 자산 후보 × 슬롯 변형 매트릭스
4. **4안 자동 제작 ▶** 클릭
5. 슬롯 페이지(`1:1245`)의 좌하단(`y=4000`)에 N안이 자동 생성됨
...
- ⚠️ **보조 이미지(데코·Memoji) 동적 배치 미적용** — [[1-3-1. 이미지 규칙]] §1.2.a 알고리즘 미구현 (다음 버전)
