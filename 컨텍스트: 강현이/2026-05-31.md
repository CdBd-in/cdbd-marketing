...
> **이 문서의 역할**: 이전 세션의 결정사항·진행 상황·다음 우선순위를 압축. 새 세션 시작 시 [[1. 블로그/썸네일/0. 목표와 비전]] → 이 문서 → 작업.
...
| 파일 | 역할 |
|------|------|
| `자동화 도구/cdbd-thumbnail-plugin/code.js` | Figma 플러그인 메인 (createVariants, applyMockup, apply3DIcon, fillText) |
| `자동화 도구/cdbd-thumbnail-plugin/ui.html` | 플러그인 UI + 검증 예시 자동 채우기 (A/D) |
| `자동화 도구/cdbd-thumbnail-plugin/manifest.json` | Figma 플러그인 매니페스트 |
| `자동화 도구/cdbd-thumbnail-plugin/decorations.json` | 자산 메타 카탈로그 ⭐ (slug→hash·url·tier·match) |
| `자동화 도구/cdbd-thumbnail-plugin/auto-fetch-decoration.py` | Thiings 자동 다운로드 |

### 문서 (디자인 가이드)
| 파일 | 핵심 변경 |
|------|----------|
| [[1. 블로그/썸네일/1. 디자인 가이드/1-1. 스타일]] | 강조색 단순화 (Purple 고정) / D 텍스트 폭 350 / 하단 y 자동 보정 명시 |
...
### P2. 자동화 확장
- [ ] **B 유형** (목업+에디터) 자동화 — VISUAL_SLOT + EDITOR_SLOT 동시 처리. Playwright 캡쳐 인프라(`자동화 도구/cdbd-capture/`) 이미 구축됨.
